...
generating_report <- function(input_json){
  ...
  #C,R,RC
  if(suffix =="C"|suffix=="R"|suffix=="RC"|(suffix_length >7 & suffix_length<10)){
    rmarkdown::render(
...
#Applies the function on all jsons
...
output_file_name_ds <- paste0(args[1],"/","combined_drug_susceptiblities.csv")
